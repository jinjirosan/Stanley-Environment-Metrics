...
logging.basicConfig(
    filename='/var/log/enviroplus.log',
    filemode='a',
    format='%(asctime)s.%(msecs)03d %(levelname)-8s %(message)s',
    level=logging.INFO,
    datefmt='%Y-%m-%d %H:%M:%S')

logging.info("""airmetrics.py - Displays readings from all of Enviro plus' sensors
...
try:
    while True:
        ...
        if mode == 4:
            # variable = "oxidised"
            unit = "kOhm" # unit was kO for kilo Ohms
            data = gas.read_all()
        ...
        if mode == 10:
            # variable = "UPSvolt"
            unit = "volt"
            data = readVoltage(bus)
            display_text(variables[mode], data, unit)

        if mode == 11:
            # variable = "UPScap"
            unit = "%"
            data = readCapacity(bus)
            display_text(variables[mode], data, unit)
        ...
        if mode == 12:
            ...
            try:
                ...
                display_everything()
            raw_data = readVoltage(bus)
            save_data(10, raw_data)
            display_everything()
            raw_data = readCapacity(bus)
            save_data(11, raw_data)
            display_everything()
